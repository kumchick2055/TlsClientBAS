<div class="container-fluid">
	...
	<span id="WithPostData" style="display:none">
		<div class="col-xs-12">
			<form class="form-horizontal">
				<div class="form-group">
					<%= _.template($('#input_constructor').html())({id:"PostName", description:tr("Request Parameter Name"), default_selector: "string", disable_int:true, dont_use_top: true, help: {description: tr("Post parameter name. After setting this value you need to add it to 'Post Data' parameter by hitting '+ Add' button.")}}) %>
				</div>
			</form>
		</div>
		...
		<div class="col-xs-12">
			<form class="form-horizontal">
				<div class="form-group">
					<%= _.template($('#input_constructor').html())({id:"PostValue", description:tr("Request Parameter Value"), default_selector: "string", disable_int:true, dont_use_top: true, help: {description: tr("Post parameter value. After setting this value you need to add it to 'Post Data' parameter by hitting '+ Add' button.")} }) %>
				</div>
			</form>
		</div>
		...
		<%= _.template($('#input_constructor').html())({id:"PostDataArray", description:tr("Post Data"), default_selector: "expression", disable_int:true, disable_string:true, help: {description: tr("To add values into post data list, fill 'Request Parameter Name' and 'Request Parameter Value' and hit '+ Add' button. This list of key value parameters will be sended to server in form, which is defined in 'Content Type' parameter.")}}) %>

		<%= _.template($('#input_constructor').html())({id:"ContentType", description:tr("Content Type"), default_selector: "string", disable_int:true, value_string: "urlencode", variants:["urlencode","multipart","json"], size:3, help: {description: tr("Content type with 3 options available: urlencode, multipart, json"), examples:[{code:"urlencode",description:tr("Send data as key/value pairs.")},{code:"multipart",description:tr("Send data as key/value pairs. Values may have binary format, can be used to send files.")},{code:"json",description:tr("Send data in json format.")}]}}) %>      
	</span>
	...
	<%= _.template($('#input_constructor').html())({id:"Method", description:tr("Method"), default_selector: "string", disable_int:true, value_string: "POST", variants:["POST","PUT","PATCH"], size:3, help: {description: tr("Request method, you can put any string here.")}}) %>
